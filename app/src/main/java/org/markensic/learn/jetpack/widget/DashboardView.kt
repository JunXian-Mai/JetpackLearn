...
import org.markensic.baselibrary.global.extensions.cos
import org.markensic.baselibrary.global.extensions.dp
import org.markensic.baselibrary.global.extensions.sin
...
class DashboardView @JvmOverloads constructor(
  ...
  override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
    ...
    scalePaint.pathEffect = PathDashPathEffect(scalePath, (dashPathMeasure.length - scaleWidth) / scaleCount, 0f, PathDashPathEffect.Style.ROTATE)
  }

  override fun onDraw(canvas: Canvas) {
    canvas.drawPath(dashPath, dashPaint)
    canvas.drawPath(dashPath, scalePaint)
    canvas.drawLine(arcRectF.centerX(), arcRectF.centerY(),
      getPointEndX(5),
      getPointEndY(5),
      pointPaint)
  }

  private fun getPointEndX(scaleIndex: Int) = arcRectF.centerX() + pointLength.cos(scaleIndex * unitAngle + startAngle)

  private fun getPointEndY(scaleIndex: Int)  = arcRectF.centerY() + pointLength.sin(scaleIndex * unitAngle + startAngle)
}