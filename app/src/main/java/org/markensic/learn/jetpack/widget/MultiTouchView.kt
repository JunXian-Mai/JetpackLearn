...
class MultiTouchView(context: Context, attrs: AttributeSet) : View(context, attrs) {
  ...
  private val targetWidth = 200.dp
  private val avatar = BitmapFactory.decodeResource(resources, R.drawable.avatar).compressInWidth(targetWidth.toInt())
  ...
  override fun onTouchEvent(event: MotionEvent): Boolean {
    ...
    when(event.actionMasked) {
      ...
      MotionEvent.ACTION_MOVE -> {
        translateX = event.getX(event.findPointerIndex(trackPointId)) - downX + preTranslateX
        translateY = event.getY(event.findPointerIndex(trackPointId)) - downY + preTranslateY
        invalidate()
      }
