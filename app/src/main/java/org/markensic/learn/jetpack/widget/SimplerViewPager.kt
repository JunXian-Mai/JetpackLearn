  ...
  override fun onTouchEvent(event: MotionEvent): Boolean {
    ...
    when(event.actionMasked) {
      ...
      MotionEvent.ACTION_MOVE -> {
        ...
        val dx = (downX - event.x + downScroll).coerceAtLeast(0f).coerceAtMost(dMaxX.toFloat())
        scrollTo(dx.toInt(), 0)
      }
      MotionEvent.ACTION_UP -> {
        ...
        val isFling = (abs(velocityX) > minVelocity) && abs(downScroll - preDownScroll) > abs(width / 2)
        if (isFling) {
          Log.d(":test", "currentPage: $currentPage")
          val scrollPage = computeScrollPage(velocityX)
          Log.d(":test", "scrollPage: $scrollPage")
          val targetPage = (currentPage + scrollPage).coerceAtLeast(0).coerceAtMost(childCount - 1)
